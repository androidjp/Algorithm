...
import com.jp.algorithm.basic.annotaion.Person;
 ...
 * ，而且由于RSA算法进行的都是大数计算，使得RSA最快的情况也比DES慢上倍
...
public class RsaUtil {
    ...
    public static void main(String[] args){
        ...
        String jsonData= new JSONObject(personList).toString();

        System.out.println("加密前json数据 ---->"+jsonData);
        System.out.println("加密前json数据长度 ---->"+jsonData.length());
        ...
        try {
            encryptBytes = encryptByPublicKeyForSpilt(jsonData.getBytes(),publicKey.getEncoded());
        } catch (Exception e) {
            e.printStackTrace();
        }
        long end=System.currentTimeMillis();
        System.out.println("公钥加密耗时 cost time---->"+(end-start));
        String encryStr=Base64Util.base64(new String(encryptBytes));
        ...
        try {
            decryptBytes = decryptByPrivateKeyForSpilt(Base64Util.base64Decode(encryStr),privateKey.getEncoded());
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        encryStr= Base64Util.base64(new String(encryptBytes));
        System.out.println("加密后json数据 --2-->"+encryStr);
        System.out.println("加密后json数据长度 --2-->"+encryStr.length());
        //公钥解密
        start=System.currentTimeMillis();
        try {
            decryptBytes=  decryptByPublicKeyForSpilt(Base64Util.base64Decode(encryStr),publicKey.getEncoded());
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    /**
     * 实现分段加密：搞定了填充方式之后又自信的认为万事大吉了
     * ，可是意外还是发生了，RSA非对称加密内容长度有限制
